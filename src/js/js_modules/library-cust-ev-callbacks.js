...
export default {
	'tab-is-clicked': function (ev) {
		...
		ev.detail.stateLib.setStateValue.call(
			ev.detail.stateLocalLib, 'xpay_fiat_quan', '', time
		);
	...
	'end-counting': function (ev) {
		...
		ev.detail.stateLib.setStateValue.call(
			ev.detail.stateLocalLib, 'xpay_conting_type', null, time
		);
	...
	'end-timer': function (ev) {
		...
		ev.detail.stateLib.setStateValue.call(
			ev.detail.stateLocalLib, 'xpay_time', null, time
		);
		ev.detail.stateLib.setStateValue.call(
	...
	'clean-transaction': function(ev) {
		...
		ev.detail.stateLib.setStateValue.call(
			ev.detail.stateLocalLib, 'xpay_conting_rate', null, time
		);
	...
	'to-server-validation': function(ev) {
		...
		function sendSellData() {
			...
			function getSellData() {
				let data = {};
				data.own = state.getStateValue.call(local, 'xpay_crypto');
				data.want = state.getStateValue.call(local, 'xpay_fiat');
				...
				data.iban = state.getStateValue.call(db, 'xpay_sell_iban');
				return data;
			}
		}
	...
	'transaction-is-started': function(ev) {
		...
		ev.detail.stateLib.setStateValue.call(
			ev.detail.stateLocalLib, 'xpay_conting_type', null, ev.detail.time
		);
	...
	'finish-transaction': function (ev) {
		...
		function getBuyData(ev) {
			...
			data["fromCurrency"] = state.getStateValue.call(local, 'xpay_fiat');
			data["toCurrency"] = state.getStateValue.call(local, 'xpay_crypto');
