...
import setLoginButtonDescrip from './logic_setLoginButtonDescrip.js';
...
export default function() {
	...
	const MESSAGES = {
			createOK: 'Your account has been created.<br>We have sent you an email with a confirmation link to the email address you specified as your account name. Please confirm your registration.',
	};
	...
	setLoginButtonDescrip.call(this);

	if (getPageName() == 'account.html') {
		// log-out
		this.login_button.addEventListener('click', ev => {
			stateLib.setStateValue.call(stateLocalLib, 'xpay_user', null, stateLib.getTime());
		...
		// open modal window (if the login button is pressed)
		this.login_button.addEventListener('click', ev => {
		...
		// if any tab in login window is pressed
		...
		// if the "close" button in login window is pressed
		this.login_close_button.addEventListener('click', ev => {
			closeAuthenticationWindow.call(this);
		});

		// if the "do" button in login window is pressed
